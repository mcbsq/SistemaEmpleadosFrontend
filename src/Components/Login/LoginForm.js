...
const RegField = ({ label, name, type = "text", placeholder = "", onChange, autoComplete, col = "col-6", hint, readOnly, onFocus }) => (
  <div className={`rf-field ${col}`}>
    <label className="rf-label">{label}</label>
    <input
      name={name} type={type} className="rf-input"
      placeholder={placeholder} onChange={onChange}
      autoComplete={autoComplete || "off"}
      readOnly={readOnly} onFocus={onFocus}
    />
    {hint && <span style={{ fontSize: "0.7rem", color: "var(--hr-muted, #86868b)", marginTop: 3 }}>{hint}</span>}
  </div>
);

const TelField = ({ label, name, col = "col-6", onChange: parentOnChange }) => {
  const [display, setDisplay] = useState("");
  const handleChange = (e) => {
    const formatted = formatTelefono(e.target.value);
    setDisplay(formatted);
    parentOnChange({ target: { name, value: formatted } });
  };
  return (
    <div className={`rf-field ${col}`}>
      <label className="rf-label">{label}</label>
      <input name={name} type="tel" className="rf-input" placeholder="55 1234 5678"
        value={display} onChange={handleChange} autoComplete="off" maxLength={12} />
    </div>
  );
};

const SelectField = ({ label, name, options = [], col = "col-6", onChange, value = "" }) => (
  <div className={`rf-field ${col}`}>
    <label className="rf-label">{label}</label>
    <select name={name} className="rf-input" value={value} onChange={onChange}
...
function LoginForm({ onClose }) {
  ...
  const [formUser, onUserChange] = useFormState({ user: "", password: "" });
  ...
  const [formPers, setFormPers]  = useState({ nombreContacto: "", parenstesco: "", telefonoContacto: "", correoContacto: "" });
  ...
  const step1 = async () => {
    ...
    setLoading(true);
    try {
      const payload = { ...formEmp, Fotografia: filesContent[0]?.content ?? "", depto_id: "Sin Asignar", Cargo: "Personal", estado: "pendiente" };
  ...
  const step2 = async () => {
    if (!formUser.user.trim()) { setAlert({ msg: "Ingresa un nombre de usuario.", ok: false }); return; }
    if (!formUser.password)    { setAlert({ msg: "Ingresa una contraseña.", ok: false }); return; }
    if (!pwdChecks.every(Boolean)) { setAlert({ msg: "La contraseña no cumple los requisitos de seguridad.", ok: false }); return; }
    setLoading(true);
    try {
      await usuarioService.create({ user: formUser.user.trim(), password: formUser.password, role: "EMPLOYEE", empleado_id: empleadoId });
      setStep(3);
    } catch (err) {
        ...
        <div className="rf-body">
          ...
          {step === 1 && (
            <div className="rf-section">
              <div className="rf-avatar-zone">
                <button type="button" className={`rf-avatar-btn ${filesContent.length > 0 ? "has-photo" : ""}`} onClick={openFilePicker} aria-label="Foto de perfil">
                  {filesContent[0]
              ...
              <div className="rf-field col-12">
                ...
                <div className="rf-pwd-wrap">
                  <input
                    ...
                    readOnly={true}
                    onFocus={() => { if (pwdInputRef.current) pwdInputRef.current.removeAttribute("readonly"); }}
                  />
              ...
              <div className="rf-checklist">
                {PWD_RULES.map((r, i) => (
                  <div key={i} className={`rf-check-item ${pwdChecks[i] ? "ok" : ""}`}>
                    <span className="rf-check-dot" />{r.label}
                  </div>
                ))}
              </div>
            </div>
          ...
          {step === 3 && (
            <div className="rf-section">
              ...
              <div className="rf-grid">
                <TelField label="Teléfono Celular" name="TelCelular" onChange={onContChange} />
                <TelField label="Teléfono Fijo"    name="TelFijo"    onChange={onContChange} />
                <RegField label="WhatsApp (ID)" name="IdWhatsApp" placeholder="5215512345678" onChange={onContChange}
                  hint="Número con código de país, sin + ni espacios" />
                <RegField label="Telegram" name="IdTelegram" placeholder="@tu_usuario" onChange={onContChange}
                  hint="Tu @usuario de Telegram, o ID numérico" />
              </div>

              <p className="rf-section-title" style={{ marginTop: 20 }}>Contacto de emergencia</p>
              <div className="rf-grid">
                <RegField label="Nombre completo" name="nombreContacto" placeholder="Ej: María López"
                  onChange={e => setFormPers(p => ({ ...p, nombreContacto: e.target.value }))} col="col-12" />
                <SelectField
                  label="Parentesco" name="parenstesco"
                  options={catalogos.parentesco} col="col-6"
                  value={formPers.parenstesco}
                  onChange={e => setFormPers(p => ({ ...p, parenstesco: e.target.value }))}
                />
                <TelField label="Teléfono" name="telefonoContacto"
                  onChange={e => setFormPers(p => ({ ...p, telefonoContacto: e.target.value }))} />
                <RegField label="Correo (opcional)" name="correoContacto" type="email"
                  onChange={e => setFormPers(p => ({ ...p, correoContacto: e.target.value }))} col="col-12" />
              </div>
              ...
              <div className="rf-grid">
                ...
                <RegField label="Código Postal"   name="CodigoP"     onChange={onDirChange} col="col-4" placeholder="12345" />
              </div>
            </div>
          )}
